...
class BiLSTM(nn.Module):
    def __init__(self, vocab_size, emb_size, hidden_size, out_size, drop_out=0.2, use_pretrained_w2v=False):
    ...
    def forward(self, x):
        ...
        scores, _ = self.bilstm(emb)
        # scores = self.fc(output[:, -1, :]) # 句子最后时刻的 hidden state
        return scores
